...
describe('Symlink Handling - Integration Tests', () => {
  it('should handle large directory structure with symlinks without infinite loops', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hashwalk-symlink-test-'));
    ...
    try {
      ...
      while (directories.length < 200) {
        const parentDir = directories[Math.floor(Math.random() * Math.min(directories.length, 50))];
        if (!parentDir) break;
      ...
      assert.ok(directories.length >= 200, 'Should have created at least 200 directories');
